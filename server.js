        ...
        if(req.session.subject){
          ...
          client.get('https://api.wit.ai/message?q=' + encodeURIComponent(searchedItem), wit, function(response){
            ...
            if(subjectLocal == "izintarih" && response.entities.day && response.entities.month && response.entities.year){
              global.vacationFlag = response.entities.day[0].value;
              instanceMongoQueries.updateOne(req.headers.authorization && global[req.headers.authorization.split(" ")[1]] ? global[req.headers.authorization.split(" ")[1]].defaultAuthorizationToken : global.defaultAuthorizationToken, 'configuration', {}, {$set : {'vacationFlag' : global.vacationFlag}}, function(resp){});
