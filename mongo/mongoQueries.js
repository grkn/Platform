var mongoQueries = class MongoQueries {
  ...
  createCollection(collectionName, callback){
    this.db.db('platform').createCollection(collectionName, function(err, res) {
      if (err) throw err;
      callback(res, err);
    });
  }
  ...
  deleteMany(collectionName,query, callback){
    this.db.db('platform').collection(collectionName).deleteMany(query, function(err, obj) {
      if (err) throw err;
      callback(obj);
    });
  }
